...
<!DOCTYPE html>  
 ...
 <title> Menampilkan grafik profit </title>
		...
		$(document).ready(function() { generatetable(); scroll(x);});
		</script>
...
var timePassed;
var target = ($('#box')[0].scrollHeight)-$div.height()
var speed = 1000/target;
var move = "down";
var x = '1';
function scroll(x)
{
	if (x=='0')
	{
		...
		clearTimeout(scrollbottom);
	}
	else if (x=='1')
	{
	$div.animate({ scrollTop: target}, 10000);
	scrollbottom = setTimeout(function() {
			var position = $div.scrollTop();
			$div.animate({scrollTop:0}, 10000);
    },10000);
	scrolltopbottom = setInterval(function(){
		$div.animate({ scrollTop: target }, 10000);
		scrollbottom = setTimeout(function() {
			var position = $div.scrollTop();
			$div.animate({scrollTop:0}, 10000); 
